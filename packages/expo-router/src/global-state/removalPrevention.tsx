...
export const GlobalRoutesWithRemovalPreventedContext = createContext<readonly string[] | undefined>(
  undefined
);
...
function RoutesWithRemovalPreventedProvider({ children }: PropsWithChildren) {
  ...
  const preventedRouteKeys = useMemo(() => [...preventedRoutes.keys()], [preventedRoutes]);
...
export function useRoutesWithRemovalPrevented() {
  return use(GlobalRoutesWithRemovalPreventedContext) ?? [];
}
...
export function isRouteRemovalPrevented(
  ...
  const preventedRouteKeys = new Set(routesWithRemovalPrevented);
